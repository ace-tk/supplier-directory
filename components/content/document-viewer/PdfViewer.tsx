...
import { ChevronLeft, ChevronRight, ZoomIn, ZoomOut, Download, Maximize2, Minimize2, StretchHorizontal, Square } from "lucide-react";
...
export function PdfViewer({ file, variant = "compact" }: { file: DraftAttachment; variant?: ViewerVariant }) {
  ...
  // Bumped by the error state's "Retry" action to re-run the load effect
  // below without needing a fresh PdfViewer instance.
  ...
  useEffect(() => {
    ...
    // re-run trigger (the "Retry" action below).
  ...
  return (
    ...
    >
      {loading ? <LoadingState /> : <canvas ref={canvasRef} className="shadow-card rounded" />}
    </ViewerChrome>
  );
}
