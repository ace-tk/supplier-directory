...
/** Admin equivalent of app/(buyer)/buyer/mood-board/all/page.tsx — same
 * secondary "browse all boards" fallback, not linked from the sidebar. */
export default async function AdminAllMoodBoardsPage() {
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Button render={<Link href="/mood-board/new" />} nativeButton={false} className="gap-1.5">
      ...
      {boards.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-5">
          {boards.map((b) => (
            <Link key={b.id} href={`/mood-board/${b.id}`} className="block rounded-2xl border border-border bg-card overflow-hidden hover:shadow-md transition-shadow">
