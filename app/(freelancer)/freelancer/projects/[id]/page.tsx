...
export default async function FreelancerProjectWorkspacePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const project = await getProjectWorkspace(id);
  if (!project) notFound();
  return <ProjectWorkspace project={project} backHref="/freelancer/projects" />;
}
