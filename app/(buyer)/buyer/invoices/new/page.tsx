...
export default async function NewBuyerInvoicePage({
  ...
  if (!user) redirect("/login?from=/buyer/invoices/new");
  ...
  if (source) {
    ...
    return <InvoiceEditor basePath="/buyer/invoices" type={invoiceType} initialInvoice={null} sourceInvoice={sourceInvoice} />;
  }

  const invoiceType = parseStandaloneInvoiceType(type, false);
  return <InvoiceEditor basePath="/buyer/invoices" type={invoiceType} initialInvoice={null} />;
}
