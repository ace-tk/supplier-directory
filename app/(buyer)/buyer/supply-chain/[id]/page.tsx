...
export default async function BuyerSupplyChainWorkspacePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const chain = getSupplyChainById(id);
  if (!chain) notFound();

  return <SupplyChainWorkspace chain={chain} basePath="/buyer/supply-chain" />;
}
