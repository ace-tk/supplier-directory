...
export default async function NewSupplierInvoicePage({
  ...
  if (!user) redirect("/login?from=/supplier/invoices/new");
  ...
  if (source) {
    ...
    return <InvoiceEditor basePath="/supplier/invoices" type={invoiceType} initialInvoice={null} sourceInvoice={sourceInvoice} />;
  }

  const invoiceType = parseStandaloneInvoiceType(type, false);
  return <InvoiceEditor basePath="/supplier/invoices" type={invoiceType} initialInvoice={null} />;
}
